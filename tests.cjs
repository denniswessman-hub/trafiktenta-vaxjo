...
const superQuestions=data.questions.filter(q=>q.exam==='superquiz');assert.equal(superQuestions.length,50);
for(const q of superQuestions){assert.equal(q.choices.length,0);assert.equal(q.title,'Fråga '+q.original);assert(q.sourceLabel);assert(q.sourceItem);assert.equal(q.reviewed,'2026-09-21');assert(!q.teacher);assert(!q.answer.includes('[Riksdagen]'));assert(!q.answer.includes('**'));}
for(const [category,n] of [['Trafikbrottslagen',18],['Fordon, skatt och behörighet',10],['Polisens trafikstrategi',4],['Fordonsdefinitioner',18]])assert.equal(superQuestions.filter(q=>q.category===category).length,n);
const superSelectText=app.slice(app.indexOf('function superSelection('),app.indexOf('function superquiz()'));
const superContext={SUPER:superQuestions,state:{ratings:{'super-01':'hard'}},dueQuestions:()=>[superQuestions[1]]};vm.runInNewContext(superSelectText,superContext);
for(const [topic,n] of [['all',50],['hard',1],['due',1],['Fordonsdefinitioner',18],['Polisens trafikstrategi',4]])assert.equal(superContext.superSelection(topic).length,n);
assert(app.indexOf('Öppna SUPERQUIZ')<app.indexOf('Välj ditt snabbquiz'));
const superSaved=L.normalize({ratings:{'super-01':'known'},drafts:{},stars:['super-01'],session:{ids:['super-01'],index:0,mode:'SUPERQUIZ',recall:true,answers:{'super-01':{revealed:true,rating:'known'}}}},ids);
assert.equal(superSaved.session.answers['super-01'].rating,'known');assert.equal(superSaved.session.recall,true);
